...
class DataReader(object):
    ...
    def ReadData(self):
        ...
        if train_file.exists():
            f = open(train_file, "r")
            X_Raw = np.zeros((506, 13))
            Y_Raw = np.zeros((506, 1))
            cnt_line = 0
            for line in f.readlines():
                raw_data = np.array(list(map(float, line.strip().split())))
                X_Raw[cnt_line, :] = raw_data[0:13]
                Y_Raw[cnt_line, 0] = raw_data[13]
                cnt_line += 1
            self.XRaw = X_Raw
            self.YRaw = Y_Raw
            self.num_train = cnt_line
